...
`envoy-control-plane` (`security_txt.go`) can serve an [RFC 9116](https://www.rfc-editor.org/rfc/rfc9116)
`security.txt` for every `routes.csv` domain — at both the canonical
`/.well-known/security.txt` and the legacy `/security.txt` — answered
directly by Envoy, no upstream, no scoring. Off entirely unless
`SECURITY_TXT_CONTACT` is set (RFC 9116 makes Contact mandatory; see
`.env.example` for the rest of the fields, all optional).
