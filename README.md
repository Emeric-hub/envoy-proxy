...
```
 ...
 envoy/ssl/  ───watch──▶│  ssl/ into listeners/routes/clusters,      │
  ...
  client ──HTTP/HTTPS/HTTP3──▶ envoy ──ext_authz──▶ scoring-service ──▶ decision
                                 ...
                                 │                        └────────▶ local heuristic
                                 │
                                 └──routes.csv target──▶ backend / default-site
...
| `default-site`         | Static catch-all for any domain not in routes.csv                   |
...
| `ssl`         | Terminate TLS for this domain (needs a cert — see `generate-cert.sh`)   |
